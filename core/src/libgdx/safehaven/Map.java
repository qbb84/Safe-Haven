...
public abstract class Map implements AudioSubject {
	...
	private void setClosestStartPosition(final Vector2 position) {
		Gdx.app.debug(TAG,
			"setClosestStartPosition INPUT: (" + position.x + "," + position.y + ") " + _currentMapType.toString());
		...
		for (MapObject object : _spawnsLayer.getObjects()) {
			...
			if (objectName.equalsIgnoreCase(PLAYER_START)) {
				...
				if (distance < shortestDistance || shortestDistance == 0) {
					...
					Gdx.app.debug(TAG,
						"closest START is: (" + _closestPlayerStartPosition.x + "," + _closestPlayerStartPosition.y +
