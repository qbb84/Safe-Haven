...
public class PlayerPhysicsComponent extends PhysicsComponent {
	...
	private void selectMapEntityCandidate(MapManager mapMgr) {
		...
		for (Entity mapEntity : _tempEntities) {
			...
			//Gdx.app.debug(TAG, "Entity Candidate Location " + "(" + mapEntityBoundingBox.x + "," +
			// mapEntityBoundingBox.y + ")");
			if (mapEntity.getCurrentBoundingBox().contains(_mouseSelectCoordinates.x, _mouseSelectCoordinates.y)) {
				//Check distance
				_selectionRay.set(_boundingBox.x, _boundingBox.y, 0.0f, mapEntityBoundingBox.x, mapEntityBoundingBox.y
					, 0.0f);
				...
				if (distance <= _selectRayMaximumDistance) {
					...
					notify(_json.toJson(mapEntity.getEntityConfig()),
