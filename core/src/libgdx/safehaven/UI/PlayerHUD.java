...
import libgdx.safehaven.audio.AudioManager;
import libgdx.safehaven.audio.AudioObserver;
import libgdx.safehaven.audio.AudioSubject;
import libgdx.safehaven.battle.BattleObserver;
import libgdx.safehaven.dialog.ConversationGraph;
import libgdx.safehaven.dialog.ConversationGraphObserver;
import libgdx.safehaven.profile.ProfileManager;
import libgdx.safehaven.profile.ProfileObserver;
import libgdx.safehaven.quest.QuestGraph;
import libgdx.safehaven.screens.MainGameScreen;
import libgdx.safehaven.sfx.ClockActor;
import libgdx.safehaven.sfx.ScreenTransitionAction;
import libgdx.safehaven.sfx.ScreenTransitionActor;
import libgdx.safehaven.sfx.ShakeCamera;
		...
		switch (event) {
			case PROFILE_LOADED:
				...
				if (firstTime) {
					...
					InventoryUI.populateInventory(_inventoryUI.getInventorySlotTable(), itemLocations,
						_inventoryUI.getDragAndDrop(), InventoryUI.PLAYER_INVENTORY, false);
					profileManager.setProperty("playerInventory",
						InventoryUI.getInventory(_inventoryUI.getInventorySlotTable()));
					...
					Array<InventoryItemLocation> inventory = profileManager.getProperty("playerInventory",
						Array.class);
					InventoryUI.populateInventory(_inventoryUI.getInventorySlotTable(), inventory,
						_inventoryUI.getDragAndDrop(), InventoryUI.PLAYER_INVENTORY, false);
					...
					if (equipInventory != null && equipInventory.size > 0) {
						_inventoryUI.resetEquipSlots();
						InventoryUI.populateInventory(_inventoryUI.getEquipSlotTable(), equipInventory,
							_inventoryUI.getDragAndDrop(), InventoryUI.PLAYER_INVENTORY, false);
					}
			...
			case SAVING_PROFILE:
				...
				profileManager.setProperty("playerEquipInventory",
					InventoryUI.getInventory(_inventoryUI.getEquipSlotTable()));
		...
		switch (event) {
			case LOAD_CONVERSATION:
				...
				if (config.getItemTypeID().equalsIgnoreCase(InventoryItem.ItemTypeID.NONE.toString())) {
					EntityConfig configReturnProperty = ProfileManager.getInstance().getProperty(config.getEntityID(),
						EntityConfig.class);
	...
	public void onNotify(ConversationGraph graph, ConversationCommandEvent event) {
		switch (event) {
			case LOAD_STORE_INVENTORY:
				...
				Array<InventoryItemLocation> inventory =
					InventoryUI.getInventory(_inventoryUI.getInventorySlotTable());
			...
			case RETURN_QUEST:
				...
				EntityConfig configReturnProperty =
					ProfileManager.getInstance().getProperty(configReturn.getEntityID(), EntityConfig.class);
				...
				if (_questUI.isQuestReadyForReturn(questID)) {
					notify(AudioObserver.AudioCommand.MUSIC_PLAY_ONCE,
						AudioObserver.AudioTypeEvent.MUSIC_LEVEL_UP_FANFARE);
	...
	public void onNotify(String value, StoreInventoryEvent event) {
		switch (event) {
			...
			case PLAYER_INVENTORY_UPDATED:
				...
				InventoryUI.populateInventory(_inventoryUI.getInventorySlotTable(), items,
					_inventoryUI.getDragAndDrop(), InventoryUI.PLAYER_INVENTORY, false);
				break;
			default:
				break;
	...
	public void onNotify(Entity enemyEntity, BattleEvent event) {
		switch (event) {
			...
			case OPPONENT_DEFEATED:
				...
				int goldReward =
					Integer.parseInt(enemyEntity.getEntityConfig().getPropertyValue(EntityConfig.EntityProperties.ENTITY_GP_REWARD.toString()));
				_statusUI.addGoldValue(goldReward);
				int xpReward =
					Integer.parseInt(enemyEntity.getEntityConfig().getPropertyValue(EntityConfig.EntityProperties.ENTITY_XP_REWARD.toString()));
			...
			case PLAYER_USED_MAGIC:
				notify(AudioObserver.AudioCommand.SOUND_PLAY_ONCE,
					AudioObserver.AudioTypeEvent.SOUND_PLAYER_WAND_ATTACK);
